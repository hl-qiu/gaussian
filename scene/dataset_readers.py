...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        uid = intr.id
        R = np.transpose(qvec2rotmat(extr.qvec))  # qvec2rotmat(extr.qvec)得到的是w2c中的R; 转置之后得到的是c2w中的R
        T = np.array(extr.tvec)     # 得到w2c中的平移向量

        if intr.model == "SIMPLE_PINHOLE":
            focal_length_x = intr.params[0]
            FovY = focal2fov(focal_length_x, height)    # Y方向(竖直方向)的视角
            FovX = focal2fov(focal_length_x, width)     # X方向(水平方向)的视角
...
def storePly(path, xyz, rgb):
    ...
    if xyz is None and rgb is None:
        # 随机s生成坐标数组
        num_points = 300000
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    # cam_infos_unsorted = readCamerasFromTransforms(path, 'transforms.json', False, extension="")
    ...
    # 保存train和test数据集对应的位姿
    # train_poses = np.array([np.hstack((item.R, np.reshape(item.T, (-1, 1)))) for item in train_cam_infos])
    # test_poses = np.array([np.hstack((item.R, np.reshape(item.T, (-1, 1)))) for item in test_cam_infos])
    # np.save(path + '/poses.npy', train_poses)
    # np.save(path + '/query_pose.npy', test_poses)
